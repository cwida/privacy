#!/usr/bin/env Rscript
# Paper-style DP-SASS utility panel over output-bound multiplier and m.
...
if (length(args) < 1) {
  stop("Usage: Rscript plot_sass_m_bound_panel.R results.csv [output_png] [dataset=tpch] [delta=1e-6]")
}

input_csv <- args[1]
output_png <- if (length(args) >= 2) args[2] else "benchmark/dp/tpch_sass_m_bound_panel_paper.png"
...
plot_data <- raw %>%
  ...
  group_by(query, bound_multiplier, dp_sass_m) %>%
  summarize(
...
p <- ggplot(plot_data, aes(x = bound_label, y = plot_error, group = m_label, color = m_label)) +
  ...
  labs(x = "Output-bound multiplier", y = "Median error vs. full answer (%)", color = "SAA samples") +
