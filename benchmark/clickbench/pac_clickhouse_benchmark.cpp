...
static vector<string> SplitTopLevelComma(const string &text) {
	vector<string> parts;
	int depth = 0;
	size_t start = 0;
...
static string ExtractSelectClause(const string &query) {
	string lower = ToLowerAscii(query);
	size_t select_pos = FindKeywordOutsideParens(lower, "select ");
	size_t from_pos = FindKeywordOutsideParens(lower, " from ");
	if (select_pos == string::npos || from_pos == string::npos || from_pos <= select_pos + 7) {
		return "";
	}
	return Trim(query.substr(select_pos + 7, from_pos - (select_pos + 7)));
}
...
static string ExtractGroupByClause(const string &query) {
	string lower = ToLowerAscii(query);
	size_t group_pos = FindKeywordOutsideParens(lower, " group by ");
	if (group_pos == string::npos) {
		return "";
	}
	size_t start = group_pos + string(" group by ").size();
	size_t end = query.size();
	vector<string> terminators = {" having ", " order by ", " limit ", " offset "};
	for (auto &term : terminators) {
		size_t p = FindKeywordOutsideParens(lower, term, start);
		if (p != string::npos) {
			end = std::min(end, p);
		}
	}
	string clause = Trim(query.substr(start, end - start));
	if (!clause.empty() && clause.back() == ';') {
		clause.pop_back();
	}
	return Trim(clause);
...
static string StripSelectAlias(const string &select_item, string &alias) {
	alias.clear();
	string item = Trim(select_item);
	string lower = ToLowerAscii(item);
	size_t as_pos = FindKeywordOutsideParens(lower, " as ");
	if (as_pos != string::npos) {
		alias = Trim(item.substr(as_pos + 4));
		return Trim(item.substr(0, as_pos));
	}
	...
	if (pos > 0 && pos <= end) {
		string candidate = item.substr(pos, end - pos + 1);
		string expr = Trim(item.substr(0, pos));
		if (!expr.empty()) {
			alias = candidate;
			return expr;
		}
...
static string ResolveGroupExpression(const string &group_expr, const vector<string> &select_exprs,
                                     const std::map<string, string> &alias_map) {
	string expr = Trim(group_expr);
	if (expr.empty()) {
		return expr;
	}
	bool numeric = std::all_of(expr.begin(), expr.end(), [](unsigned char c) { return std::isdigit(c); });
	if (numeric) {
		idx_t pos = static_cast<idx_t>(std::stoll(expr));
		if (pos > 0 && pos <= select_exprs.size()) {
			return select_exprs[pos - 1];
		}
	}
	auto entry = alias_map.find(ToLowerAscii(expr));
...
static string BuildGroupBoundQuery(const string &query) {
	string from_where = ExtractFromWhereClause(query);
	string group_by = ExtractGroupByClause(query);
	if (from_where.empty()) {
		return "";
	}
	if (group_by.empty()) {
	...
	for (auto &item : select_items) {
		string alias;
		string expr = StripSelectAlias(item, alias);
		select_exprs.push_back(expr);
		if (!alias.empty()) {
			alias_map[ToLowerAscii(alias)] = expr;
		}
	}

	vector<string> group_items = SplitTopLevelComma(group_by);
	string distinct_groups = "SELECT DISTINCT UserID";
	for (idx_t i = 0; i < group_items.size(); i++) {
		distinct_groups += ", ";
		distinct_groups += ResolveGroupExpression(group_items[i], select_exprs, alias_map);
		distinct_groups += " AS __dp_group_" + std::to_string(i);
	}
	distinct_groups += " " + from_where;
	return "SELECT max(group_cnt) FROM (SELECT UserID, count(*) AS group_cnt FROM (" + distinct_groups +
	       ") GROUP BY UserID)";
}

static vector<double> NormalizeBoundMultipliers(const vector<double> &multipliers) {
	vector<double> normalized;
	for (auto multiplier : multipliers) {
		if (multiplier > 0.0 && std::isfinite(multiplier)) {
			normalized.push_back(multiplier);
		}
	}
	if (normalized.empty()) {
...
static vector<double> ParseDoubleCSV(const string &list) {
...
static vector<string> ParseStringCSV(const string &list) {
...
static vector<string> NormalizeSassReleases(const vector<string> &releases) {
	vector<string> normalized;
	for (auto release : releases) {
		release = ToLowerAscii(Trim(release));
		if (release.empty()) {
			continue;
		}
		if (release != "median" && release != "average") {
			throw std::runtime_error("unknown dp_sass release: " + release);
		}
		if (std::find(normalized.begin(), normalized.end(), release) == normalized.end()) {
			normalized.push_back(release);
		}
	}
	if (normalized.empty()) {
		normalized.push_back("median");
	}
	return normalized;
}
...
struct BenchmarkQueryResult {
	...
	string dp_max_groups_contributed;
	string bound_multiplier;
	string pac_mi;
	int64_t seed = 0;
};
...
[[noreturn]] static void ChildWorkerMain(int read_fd, int write_fd, const string &db_path) {
	try {
		...
		while (true) {
			uint8_t cmd = 0;
			if (!ReadAllBytes(read_fd, &cmd, sizeof(cmd)))
				break;
			...
			if (cmd == CMD_STOP) {
				...
				uint32_t count = 0;
				if (!ReadAllBytes(read_fd, &count, sizeof(count)))
					break;
				string setup_error;
				for (uint32_t i = 0; i < count; i++) {
					uint32_t len = 0;
					if (!ReadAllBytes(read_fd, &len, sizeof(len)))
						goto done;
					string stmt(len, '\0');
					if (!ReadAllBytes(read_fd, &stmt[0], len))
						goto done;
				...
				if (!error.empty() && (error.find("FATAL") != string::npos ||
...
struct ForkWorker {
	...
	bool SendSetupStatements(const vector<string> &stmts) {
		...
		if (!WriteAllBytes(to_child_fd, &count, sizeof(count)))
			return false;
		for (auto &s : stmts) {
			uint32_t len = static_cast<uint32_t>(s.size());
			if (!WriteAllBytes(to_child_fd, &len, sizeof(len)))
				return false;
			if (!WriteAllBytes(to_child_fd, s.data(), len))
				return false;
		}
		uint8_t ok = 0;
		uint32_t err_len = 0;
	...
	SubmitResult SubmitQuery(const string &sql, double timeout_s) {
		...
		if (!WriteAllBytes(to_child_fd, &cmd, sizeof(cmd)) || !WriteAllBytes(to_child_fd, &sql_len, sizeof(sql_len)) ||
		...
		while (true) {
			...
			if (ret > 0 && (pfd.revents & POLLIN)) {
				...
				if (!result_error.empty() && (result_error.find("FATAL") != string::npos ||
...
int RunClickHouseBenchmark(const string &db_path, const string &queries_dir, const string &out_csv, bool micro,
	...
	try {
		...
		if (selected_query_count == 0) {
			throw std::runtime_error("query filter selected no ClickBench queries");
		}
		...
		// =====================================================================
		{
			...
			if (!table_exists || needs_reload) {
				if (!FileExists(parquet_path)) {
					...
					if (ret != 0) {
						...
						download_cmd = "curl -L -o \"" + parquet_path +
			...
			for (idx_t q = 0; q < queries.size(); q++) {
				...
				auto bound_start = std::chrono::steady_clock::now();
				auto bound_result = con.Query(bound_sql);
				if (!bound_result || bound_result->HasError()) {
					sum_bound_times_ms[q] = MillisecondsSince(bound_start);
				...
				if (perfect_bounds[q] > 0) {
					Log(string("Q") + std::to_string(q + 1) +
					    " perfect dp_sum_bound=" + FormatNumber(perfect_bounds[q]));
				}
			}
			...
			for (idx_t q = 0; q < queries.size(); q++) {
				...
				if (!count_result || count_result->HasError()) {
					count_bound_times_ms[q] = MillisecondsSince(count_start);
				...
				if (perfect_count_bounds[q] > 0) {
					Log(string("Q") + std::to_string(q + 1) +
					    " perfect dp_count_bound=" + FormatNumber(perfect_count_bounds[q]));
				}
			}
			...
			for (idx_t q = 0; q < queries.size(); q++) {
				...
				auto group_result = con.Query(group_sql);
				if (!group_result || group_result->HasError()) {
					group_bound_times_ms[q] = MillisecondsSince(group_start);
					string err = group_result ? group_result->GetError() : "unknown error";
					Log(string("Q") + std::to_string(q + 1) + " group-bound inference failed: " + err);
					perfect_group_bounds[q] = PositiveOrDefault(perfect_count_bounds[q], 1.0);
					continue;
				}
				auto chunk = group_result->Fetch();
				if (chunk && chunk->size() > 0) {
					auto value = chunk->GetValue(0, 0);
					if (!value.IsNull()) {
						perfect_group_bounds[q] = value.DefaultCastAs(LogicalType::DOUBLE).GetValue<double>();
					}
				}
				group_bound_times_ms[q] = MillisecondsSince(group_start);
		...
		for (int run = 1; run <= num_runs; ++run) {
			...
			for (size_t q = 0; q < queries.size(); ++q) {
				...
				// ----------------------------------------------------------
				{
					...
					result.bound_time_ms = "0";
					result.total_time_ms = FormatNumber(result.time_ms);
				...
				// ----------------------------------------------------------
				auto run_dp_mode = [&](const string &mode_name, double bound_multiplier, const string &sass_release) {
					auto bound_time_ms = [&]() {
						double value = 0.0;
						if (query_has_sum_avg[q]) {
							value += sum_bound_times_ms[q];
						}
						if (mode_name == "dp_standard" || mode_name == "dp_sass") {
							value += count_bound_times_ms[q];
							if (query_has_group_by[q]) {
					...
					if (query_has_sum_avg[q]) {
						dp_bound = PositiveOrDefault(perfect_bounds[q], 1e-9) * bound_multiplier;
						dp_setup.push_back("SET dp_sum_bound=" + FormatNumber(dp_bound) + ";");
					} else {
						dp_setup.push_back("RESET dp_sum_bound;");
					}
					if (mode_name == "dp_standard" || mode_name == "dp_sass") {
						dp_count_bound = std::max(
						    1.0, std::ceil(PositiveOrDefault(perfect_count_bounds[q], 1.0) * bound_multiplier));
						dp_setup.push_back("SET dp_count_bound=" + FormatNumber(dp_count_bound) + ";");
					}
					if (mode_name == "dp_sass") {
						dp_setup.push_back("SET dp_sass_release='" + sass_release + "';");
					...
					if (query_has_group_by[q] && (mode_name == "dp_standard" || mode_name == "dp_sass")) {
						dp_group_bound = std::max(
						    1.0, std::ceil(PositiveOrDefault(perfect_group_bounds[q], 1.0) * bound_multiplier));
					...
					result.dp_max_groups_contributed =
					    (query_has_group_by[q] && (mode_name == "dp_standard" || mode_name == "dp_sass"))
					        ? FormatNumber(dp_group_bound)
					        : "";
					result.bound_multiplier = FormatNumber(bound_multiplier);
					result.seed = seed;
					if (result.success && worker.child_pid > 0) {
					...
					if (!setup_ok) {
						...
						Log(string("Q") + std::to_string(qnum) + " " + tag + " time: " + FormatNumber(result.time_ms) +
						    " ms");
					} else {
				...
				if (modes.count("dp_standard")) {
					for (auto multiplier : bound_multipliers) {
						run_dp_mode("dp_standard", multiplier, "");
					}
				}
				if (modes.count("dp_elastic")) {
					for (auto multiplier : bound_multipliers) {
						run_dp_mode("dp_elastic", multiplier, "");
					}
				}
				if (modes.count("dp_sass")) {
					for (auto multiplier : bound_multipliers) {
						for (auto &release : sass_releases) {
							run_dp_mode("dp_sass", multiplier, release);
						}
					}
				}
				...
				if (run_naive) {
					auto run_naive_variant = [&](const string &mode_label, const string &naive_dir) {
						...
						if (!setup_ok) {
							...
							Log(string("Q") + std::to_string(qnum) + " " + tag +
							    " time: " + FormatNumber(result.time_ms) + " ms");
						} else {
							Log(string("Q") + std::to_string(qnum) + " " + tag + " ERROR: " + result.error_msg);
						}
		...
		for (const auto &r : all_results) {
			csv << r.query_num << "," << r.mode << "," << r.sass_release << "," << r.run << ","
			    ...
			    << r.dp_max_groups_contributed << "," << r.bound_multiplier << "," << r.pac_mi << "," << r.seed << "\n";
		}
		csv.close();
		...
		int total_queries = selected_query_count;
			...
			if (r.mode == "PAC") {
				if (r.success) {
					...
					if (r.error_msg.find("sample diversity") != string::npos) {
						...
						pac_crashed++;
					} else if (r.error_msg.find("PAC") != string::npos || r.error_msg.find("privacy") != string::npos ||
		...
		if (baseline_success > 0) {
			Log(string("  Avg time per successful query: ") + FormatNumber(baseline_total_time / baseline_success) +
			    " ms");
		}
		...
		if (pac_success > 0) {
			Log(string("  Avg time per successful query: ") + FormatNumber(pac_total_time / pac_success) + " ms");
		}
		...
		// =====================================================================
		{
			...
			for (auto &kv : baseline_total_per_q) {
				int qnum = kv.first;
				if (pac_count_per_q.count(qnum) == 0 || baseline_count_per_q[qnum] == 0)
					continue;
				...
				Log(string("  Q") + std::to_string(qnum) + ": " + FormatNumber(baseline_mean) + " ms -> " +
				    FormatNumber(pac_mean) + " ms (" + FormatNumber(slowdown) + "x)");
			...
			if (n_compared > 0) {
				...
				Log(string("  Worst slowdown: ") + FormatNumber(worst_slowdown) + "x (Q" + std::to_string(worst_query) +
				    ")");
			}
		}
...
int main(int argc, char **argv) {
	...
	for (int i = 1; i < argc; ++i) {
		...
		} else if (arg == "--bound-multipliers" && i + 1 < argc) {
