...
-- Q8: AdvEngineID, COUNT(*) GROUP BY AdvEngineID ORDER BY COUNT(*) DESC
WITH samples AS (
...
per_sample AS (
    SELECT s.sample_id, h.AdvEngineID AS AdvEngineID, COUNT(*) AS c
    FROM samples s JOIN hits h ON h.UserID = s.pu_key
    WHERE h.AdvEngineID <> 0
    GROUP BY s.sample_id, AdvEngineID
)
SELECT AdvEngineID,
