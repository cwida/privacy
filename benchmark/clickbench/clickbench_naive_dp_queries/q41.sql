...
-- Q41: URLHash, EventDate, COUNT(*) PageViews [...TraficSourceID IN(-1,6), RefererHash=...] GROUP BY URLHash,EventDate ORDER BY PageViews DESC LIMIT 10 OFFSET 100
WITH samples AS (
...
per_sample AS (
    SELECT s.sample_id, h.URLHash AS URLHash, h.EventDate AS EventDate, COUNT(*) AS PageViews
    FROM samples s JOIN hits h ON h.UserID = s.pu_key
    WHERE h.CounterID = 62 AND h.EventDate >= '2013-07-01' AND h.EventDate <= '2013-07-31' AND h.IsRefresh = 0 AND h.TraficSourceID IN (-1, 6) AND h.RefererHash = 3594120000172545465
    GROUP BY s.sample_id, URLHash, EventDate
)
SELECT URLHash,
       EventDate,
