...
-- Q42: WindowClientWidth, WindowClientHeight, COUNT(*) PageViews [...DontCountHits=0,URLHash=...] GROUP BY ... ORDER BY PageViews DESC LIMIT 10 OFFSET 10000
WITH samples AS (
...
per_sample AS (
    SELECT s.sample_id, h.WindowClientWidth AS WindowClientWidth, h.WindowClientHeight AS WindowClientHeight, COUNT(*) AS PageViews
    FROM samples s JOIN hits h ON h.UserID = s.pu_key
    WHERE h.CounterID = 62 AND h.EventDate >= '2013-07-01' AND h.EventDate <= '2013-07-31' AND h.IsRefresh = 0 AND h.DontCountHits = 0 AND h.URLHash = 2868770270353813622
    GROUP BY s.sample_id, WindowClientWidth, WindowClientHeight
)
