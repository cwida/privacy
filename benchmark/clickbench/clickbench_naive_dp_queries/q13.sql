...
-- Q13: SearchPhrase, COUNT(*) WHERE SearchPhrase<>'' GROUP BY SearchPhrase ORDER BY c DESC LIMIT 10
WITH samples AS (
...
per_sample AS (
    SELECT s.sample_id, h.SearchPhrase AS SearchPhrase, COUNT(*) AS c
    FROM samples s JOIN hits h ON h.UserID = s.pu_key
    WHERE h.SearchPhrase <> ''
    GROUP BY s.sample_id, SearchPhrase
)
SELECT SearchPhrase,
