...
per_sample AS (
    SELECT s.sample_id, COUNT(*) AS c
    FROM samples s JOIN hits h ON h.UserID = s.pu_key
    WHERE h.URL LIKE '%google%'
    GROUP BY s.sample_id
)
SELECT dp_aggregate(array_agg((c * 8.0)::FLOAT ORDER BY sample_id), array_agg(0.0), 1.0, 1e-6, 8, 0.0, 1000000000.0) AS c
FROM per_sample;
