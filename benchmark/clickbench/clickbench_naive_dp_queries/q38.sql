...
-- Q38: Title, COUNT(*) PageViews [...Title<>''] GROUP BY Title ORDER BY PageViews DESC LIMIT 10
WITH samples AS (
...
per_sample AS (
    SELECT s.sample_id, h.Title AS Title, COUNT(*) AS PageViews
    FROM samples s JOIN hits h ON h.UserID = s.pu_key
    WHERE h.CounterID = 62 AND h.EventDate >= '2013-07-01' AND h.EventDate <= '2013-07-31' AND h.DontCountHits = 0 AND h.IsRefresh = 0 AND h.Title <> ''
    GROUP BY s.sample_id, Title
)
SELECT Title,
