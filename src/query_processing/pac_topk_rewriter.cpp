...
void PACTopKRule::PACTopKOptimizeFunction(OptimizerExtensionInput &input, unique_ptr<LogicalOperator> &plan) {
	...
	// After TopN selects the right top-k groups by true means, and UnnoisedProj extracts
	// per-world values, we need to re-sort the results so the output is ordered correctly.
