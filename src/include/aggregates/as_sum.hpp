...
//              Accumulates directly into float[64] total using AddToTotalsSimple (ARM).
//
...
// code simple, we added an update_counter also to PAC_SUM(). The only difference is that in the Finalize() for
// PAC_AVG() we divide the counter numbers by this update_counter first.
//
...
#else
// =========================
// Integer as_sum (cascaded multi-level accumulation for SIMD efficiency)
// =========================
...
struct PacSumIntState {
	...
	// Get probabilistic totals as PAC_FLOAT[64] for finalization
	void GetTotals(PAC_FLOAT *dst) const {
		if (max_level_used < 0) {
			memset(dst, 0, 64 * sizeof(PAC_FLOAT));
			return;
		}
		PAC_FLOAT scale = std::exp2(PAC_APPROX_LEVEL_SHIFT * max_level_used);
...
#define LOWERBOUND_BITWIDTH(bits) -(static_cast<int64_t>(SIGNED) << ((bits - SIGNED) - ACCUMULATE_BITMARGIN))
	...
	// 3. update_count (required for AVG really)
	...
	// Convert SWAR packed subtotal to PAC_FLOAT[64] for finalization
	// These read from the lowest allocated level and unpack to bit-position order
	template <typename SRC_T>
	static void UnpackSWARToPacFloat(const uint64_t *swar_data, int swar_size, PAC_FLOAT *dst) {
		...
		for (int bit = 0; bit < 64; bit++) {
			...
			dst[bit] = static_cast<PAC_FLOAT>(src[src_idx]);
		}
	}

	// Get the probabilistic total as PAC_FLOAT, reading from the highest allocated level
	void GetTotals(PAC_FLOAT *dst) const {
		if (probabilistic_total128) {
			...
			const T64 *src = reinterpret_cast<const T64 *>(probabilistic_total64);
			for (int i = 0; i < 64; i++) {
				dst[i] = static_cast<PAC_FLOAT>(src[i]);
			}
		} else if (probabilistic_total32) {
			UnpackSWARToPacFloat<T32>(probabilistic_total32, 32, dst);
		} else if (probabilistic_total16) {
			UnpackSWARToPacFloat<T16>(probabilistic_total16, 16, dst);
		} else if (probabilistic_total8) {
			UnpackSWARToPacFloat<T8>(probabilistic_total8, 8, dst);
		} else {
...
// Double as_sum state is noncascading: directly aggregates float/double into double
struct PacSumDoubleState {
	...
	void GetTotals(PAC_FLOAT *dst) const {
		for (int i = 0; i < 64; i++) {
			dst[i] = static_cast<PAC_FLOAT>(probabilistic_total[i]);
		}
	}
	// Interface methods for wrapper compatibility
...
// Forward declarations for as_sum functions

// State type selection (defined in as_sum.cpp, needed by as_avg.cpp)
#ifdef PAC_NOBUFFERING
template <bool SIGNED>
...
// FlushBuffer - flushes src's buffer into dst's inner state (declared here, defined in as_sum.cpp)
template <bool SIGNED, typename WrapperT>
void PacSumFlushBuffer(WrapperT &src, WrapperT &dst, ArenaAllocator &a);
#endif

// ============================================================================
// Double-sided mode helpers (used by as_sum and as_avg finalization)
// ============================================================================
#ifndef PAC_SIGNEDSUM
...
// For int states: cast to unsigned for GetTotals
template <class State>
inline typename std::enable_if<!std::is_same<typename State::State, PacSumDoubleState>::value>::type
GetPosStateTotals(typename State::State *pos, PAC_FLOAT *buf) {
	reinterpret_cast<PacSumIntState<false> *>(pos)->GetTotals(buf);
}
...
SubtractNegStateTotals(State *wrapper, PAC_FLOAT *buf, uint64_t &key_hash, ArenaAllocator &allocator) {
	if (auto *neg = wrapper->GetNegState()) {
		...
		neg->Flush(allocator);
		neg->GetTotals(neg_buf);
		key_hash |= neg->key_hash;
		for (int j = 0; j < 64; j++) {
			buf[j] -= neg_buf[j];
		}
		return neg->update_count;
	}
	return 0;
}
...
template <class State, bool SIGNED>
inline typename std::enable_if<!SIGNED || std::is_same<typename State::State, PacSumDoubleState>::value, uint64_t>::type
SubtractNegStateTotals(State *, PAC_FLOAT *, uint64_t &, ArenaAllocator &) {
	return 0; // No neg_state for unsigned types or double states
}
#endif

// Generic finalize template (used by as_sum)
