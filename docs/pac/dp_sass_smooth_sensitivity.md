...
- MIN and MAX are supported only as bounded-domain sample medians; utility may be
  poor.
- Group release still needs careful treatment when group keys come from private
  domains. `privacy_min_group_count` is a utility/suppression control, not the
  same as the threshold formula in bounded-contribution SQL DP systems.
