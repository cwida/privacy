...
\large what we tried, what worked, and what did not}
\author{}
\date{}
...
\begin{abstract}
\noindent
We evaluate four ideas for making user-level differential privacy automatic for grouped SQL
aggregates: an $\ell_1$ per-unit norm clip replacing cell clipping plus contribution truncation;
a frozen partition set reused across queries; exponential-mechanism selection of the contribution
bound $\Cu$; and All-or-Frozen histogram thresholding. Measured against Google DP as published
(Wilson et al., PoPETs 2020) across 14 queries on three datasets, the combined stack is
$1.9\times$ better in median and up to $9.6\times$ on queries where partition selection binds.
The gain is \emph{conditional}: it requires privacy units to touch many groups, and we identify
a regime where the baseline wins by up to $1.27\times$. Two privacy bugs were found in our own
mechanism and fixed; one is demonstrated by an explicit adversary at $47\times$ the charged
$\delta$. All headline numbers moved substantially during evaluation, and \S\ref{sec:method}
argues that the methodology is the most transferable contribution.
\end{abstract}

\section{Problem and baseline}

A grouped DP query \texttt{SELECT g, SUM(v) FROM T WHERE p GROUP BY g} with a per-user privacy
unit has three channels, each needing its own budget:

\begin{enumerate}
\item \textbf{Bound selection} ($\epsilon_b$): how large can one unit's contribution be?
\item \textbf{Partition selection} ($\epsilon_\eta$): which groups may be named at all?
\item \textbf{Values} ($\epsilon_v$): the noised aggregates.
\end{enumerate}

\paragraph{Baseline.} Google DP as published: \textsc{ApproxBounds} over per-cell values in
base-2 log bins gives $U$; a single $\Cu$ randomly truncates each unit to $\Cu$ groups, applied to
\emph{both} the value and the vote channel; value sensitivity is $\Cu \cdot U$ with Laplace noise;
partition selection is a noised distinct-unit count against the Wilson threshold
$\tau = 1 - \Cu\log(2-2(1-\delta_\eta)^{1/\Cu})/\epsilon_\eta$.

Crucially, $\Cu$ is a \emph{required input} the analyst supplies
(\texttt{max\_partitions\_contributed}). Making the system automatic means choosing it without a
human.
...
\subsection{Decomposition: what the gain is actually made of}
