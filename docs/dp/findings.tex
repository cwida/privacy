...
Guessing is not a safe fallback, for two measured reasons. The penalty is \emph{severely
asymmetric} --- across our queries, setting $\Cu$ too small costs up to $38\times$ in error while
setting it too large costs at most $1.7\times$ --- and no single value works across datasets:
...
A grouped DP query spends privacy budget on three things, and every idea in this document buys one
of them at the expense of another:
...
\paragraph{How the baseline actually divides it.} Google DP charges these separately, though not as
three co-equal shares. Privacy on Beam splits the top level \emph{two} ways --- \texttt{count.go}
exposes \texttt{AggregationEpsilon} and \texttt{PartitionSelectionEpsilon} as distinct fields,
consumed by distinct calls --- and bound selection sits \emph{inside} the aggregation:
\texttt{ApproxBounds} is constructed with its own \texttt{epsilon}
(\texttt{ApproxBounds(double epsilon, \ldots)}, \texttt{SetEpsilon}) taken from the aggregation's
share. We model all three as a single three-way split $(\epsilon_b, \epsilon_\eta, \epsilon_v)$ and
tune it jointly for every arm, which is at least as favourable to the baseline as its own
two-level structure, since a nested split cannot beat a free one.
